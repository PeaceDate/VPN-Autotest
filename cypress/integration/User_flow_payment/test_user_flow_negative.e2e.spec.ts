...
describe ('Check user flow', (testData = getDataForCase(user_flow_data, 'require')) => {
    ...
    it('Check login and payment (invalid date)', () => {
        ...
        payment.clickGetIronVpnButton(':first()')
            ...
            .fillDateField(testData.invalid_date)
            .fillCvvField(testData.cvv)
            .checkPayButton(true)
            .clickPayButton()
            .checkUnsuccessfullPopap(true)
            .checkUrl(testData.url);
    });

    it('Check login and payment (invalid cvv)', () => {
        loadFixtures(user_flow_data)
        autorization
            .clickGetIronVpnButton(':first()')
            .fillEmailInput(testData.email)
        ...
        payment.clickGetIronVpnButton(':first()')
            ...
            .fillCvvField(testData.invalid_cvv)
            .checkPayButton(true)
            .clickPayButton()
            .checkUnsuccessfullPopap(true)
            .checkUrl(testData.url);
